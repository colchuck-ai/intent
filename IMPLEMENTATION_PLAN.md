...
- [ ] **T7 · Mark section optionality across spine templates** *(dep: T6; soft dep: T3)* — requirement, component, outcome, product, architecture templates. *Accept:* each section is marked required vs. omittable in-template (or a single top-of-file pointer to `structure.md`'s "Never emit an empty heading" rule), so optionality is knowable without cross-file inference.
- [ ] **T8 · Disambiguate the dual-form templates** *(dep: T6)* — `assets/templates/product.md`, `assets/templates/architecture.md`, `assets/templates/outcome-document.md`. *Accept:* the back-to-back inline vs. reference blocks under identical headings carry a "pick one form per child" instruction, eliminating the double-emission risk.
