...
2. Judge session — Trace context again, then run a validation loop on the draft that already exists from the drafting session:
   1. Run the bundle linter as the mechanical first pass: `python3 scripts/lint_intent_tree.py docs/` from the skill root. Fix every reported error before continuing. JSON on stdout; diagnostics on stderr.
   2. Validate the draft against a binary checklist — every check is pass/fail. The checklist is the good/bad rubric plus the Minimal Pattern from the subsection for the relevant element type under [elements.md](elements.md), plus the shared [Alignment check](elements.md#alignment-check). When the draft touches decisions or history, also validate against [Records](elements.md#records).
   3. Fix the failures.
   4. Repeat the validate-and-fix cycle until every check passes.
   5. Only then finalize: commit, push, and open a PR or MR.
...
Draft and Judge only. Judge runs `python3 scripts/lint_intent_tree.py docs/` first, then the same binary checklist. Finalize and commit at the end of Judge; no PR/MR and no separate reviewer.
